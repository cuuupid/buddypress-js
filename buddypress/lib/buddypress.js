...
// http://codex.buddypress.org/XML-RPC_Support
// http://codex.buddypress.org/XML-RPC_buddypress_API
...
function Client(settings) {
	...
	//console.log(`Created client ${this.username}@${parsedUrl.host}:${parsedUrl.port}${parsedUrl.path}::${this.password}`)
}
...
Client.parseUrl = function (wpUrl) {
	...
	return {
		...
		path: urlParts.path.replace(/\/+$/, "") + "/index.php?bp_xmlrpc=true",
		secure: secure
	};
};
	...
	call: function (method) {
		...
		this.rpc.methodCall(method, args, function (error, data) {
			...
			if (error.code === "ENOTFOUND" && error.syscall === "getaddrinfo") {
				error.message = "Unable to connect to buddypress.";
...
extend(Client.prototype, {
	...
	verifyConnection: function (cb) {
		this.authenticatedCall('bp.verifyConnection', fieldMap.to([this.username, this.password], 'post'), cb)
	},

	getProfile: function (cb) {
		this.authenticatedCall('bp.getProfile', cb)
	},

	getMessages: function (data, cb) {
		this.authenticatedCall('bp.getMessages', fieldMap.to([this.username, this.password], 'xx'), cb)
	},
